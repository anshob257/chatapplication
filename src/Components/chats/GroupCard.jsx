...
const GroupCard = ({ user, onClick }) => {

  const {groupActive} = useContext(SectionContext);
  ...
  return (
    <div className={`${groupActive === user.groupName ? "bg-blue-500" : "bg-white bg-opacity-[8%] hover:bg-opacity-[15%]"} flex group  transition-all duration-300 ease-linear flex-row justify-between w-[93%] py-[1vw] px-[1.25vw] rounded-[1.25vw] cursor-pointer mt-[0.75vw]`} onClick={handleClick}>
      <div className={` ${groupActive === user.groupName ? "bg-red-500 border-yellow-300 border-[2.5px]" : "border-blue-600 border-[1px]"}   w-[17%] rounded-full relative`}>
      ...
      <div className="w-[80%] flex flex-col pt-[0.1vw]">
        <div className="text-[1.2vw] font-semibold">{user.groupName}</div>
        <div className={`w-[100%] ${groupActive === user.groupName ? "text-white" : "text-darkText"} text-[1vw] overflow-hidden h-[1.75vw] mt-[0.1vw]`}>
